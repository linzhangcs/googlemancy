...
/*
* This function generates a poem from pre-defined sentence structure
* and list of words
*
*/
function init(){
...
function parseSentencs(t){
  let doc = nlp(t);
  let pos = {};
  var nouns = doc.sentences().match('#Noun').out('text').replace(/\s+/g, ',').split(',');
  var verbs = doc.sentences().match('#Verb').out('text').replace(/\s+/g, ',').split(',');
  var adjectives = doc.sentences().match('#Adjective').out('text').replace(/\s+/g, ',').split(',');
  ...
  console.log("print out nouns: " + pos);
  return pos;
}
// -- UTILITY FUNCTIONS
